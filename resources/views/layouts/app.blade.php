...
<body
    ...
    <header class="w-full max-w-[1200px] text-sm mb-6 not-has-[nav]:hidden">
        <nav class="flex items-center justify-begin gap-4 dark:text-[#EDEDEC]">
            ...
            @php($userName = auth()->user()->name)
                ...
                @else
                    <a class="hover:underline font-bold" href="/profile">{{ $userName }}</a>
                @endif
            @else
    ...
    <footer class="p-10 w-full text-center dark:text-[#EDEDEC]"><?= date('Y') ?></footer>

    @auth
        <div x-data="{ open: JSON.parse(localStorage.getItem('nav_open') || 'false') }"
    ...
    <div x-data="{ show: false, message: '' }"
        ...
        <div x-show="show" x-transition class="px-4 py-2 rounded-xl shadow-md text-sm font-medium 
                            bg-gray-700/80 text-white backdrop-blur-md">
