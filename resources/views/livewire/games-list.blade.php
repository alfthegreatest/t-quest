<div>
    <div class="mx-auto overflow-x-auto shadow rounded-lg dark:bg-gray-800 max-w-[800px]">
            ...
            <tbody>
                @foreach($games as $game)
                    <tr wire:key="game-{{ $game->id }}" class="">
                        ...
                        <td><button type="button" wire:click="edit({{ $game->id }})"
                                class="bg-gray-600 text-white px-3 py-1 rounded cursor-pointer hover:bg-gray-500">edit</button>
                        </td>
        ...
        @if($showEditGameModal)
            <div wire:click="$set('showEditGameModal', false)"
                class="fixed inset-0 bg-black/90 flex items-center justify-center z-60">
                <div wire:click.stop class="bg-gray-800 text-white rounded-lg w-96 p-6 shadow-lg">
                    ...
                    <form wire:submit.prevent="update" class="space-y-4">
                        ...
                        <input type="text" wire:model.live="title" placeholder="Title"
                            class="w-full p-2 rounded bg-gray-700 
                                                ...
                                                focus:border-blue-500 @error('title') border-red-500 ring-red-500 @enderror" value="{{ $title }}">

                        <textarea wire:model.live="description" placeholder="Description"
                            class="w-full p-2 rounded bg-gray-700 border border-gray-600 focus:outline-none focus:border-blue-500">{{ $description }}</textarea>
                        ...
                        <div class="flex justify-end gap-2 mt-4">
                            ...
                            <button type="button" wire:click="$set('showEditGameModal', false)"
