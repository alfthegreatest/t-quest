<div id="edit-level-popup">
        ...
        <div wire:click="$set('showEditLevelPopup', false)" class="overlay">
            <div wire:click.stop class="popup">
                ...
                <form wire:submit.prevent="save" class="space-y-4">
                    ...
                    <input type="text" wire:model.live.debounce.1000ms="name" placeholder="Name"
                    ...
                    <textarea wire:model.live.debounce.2000ms="description" placeholder="Description"
                    ...
                    <div class="flex-1">
                        ...
                        <input type="number" wire:model.live.debounce.1000ms="points" min="0" placeholder="0"
                            class="input-number @error('points') border-red-500 @enderror">
                    </div>


                    <div class="space-y-2">
                        <label class="block text-sm font-medium">Duration <x-field-notification
                                field="availability_time" /></label>
                        ...
                        <div class="flex items-center gap-3">
                            <div class="relative flex-1">
                                <input type="number" wire:model.live.debounce.1000ms="availability_time_days" min="0"
                                    max="364" placeholder="0"
                                    class="input-number @error('availability_time_days') border-red-500 @enderror">
                                <span class="input-suffiks">days</span>
                            </div>

                            <div class="relative flex-1">
                                <input type="number" wire:model.live.debounce.1000ms="availability_time_hours" min="0"
                                    max="23" placeholder="0"
                                    class="input-number @error('availability_time_hours') border-red-500 @enderror">
                                <span class="input-suffiks">hours</span>
                            </div>

                            <div class="relative flex-1">
                                <input type="number" wire:model.live.debounce.1000ms="availability_time_minutes" min="0"
                    ...
                    <div class="space-y-2">
                        <label class="block text-sm font-medium">Coordinates <x-field-notification
                                field="coordinates" /></label>
                        @error('coordinates')
                    ...
                    <div class="space-y-2">
                        <div class="relative">
                            <label class="label-base">Image <x-field-notification field="level_image" /></label>
                            <label
                    ...
                    <a target="_blank" href="{{ route('game.level.codes', [$gameId, $id]) }}"
                        class="inline-flex items-center gap-1 text-gray-400 hover:text-white transition mb-4">codes<svg
                    ...
                    @if(!$showDeleteLevelButtons)
                        <div class="btn-group">
                            <button type="button" wire:click.prevent="$set('showDeleteLevelButtons', true)" class="del-btn">
                                Delete level
                            </button>

                            <button type="button" wire:click="$set('showEditLevelPopup', false)" class="cancel-btn">
                                Close
                            </button>
                        </div>
                    @endif
                    ...
                    @if($showDeleteLevelButtons)
                        <div class="btn-group">
                            ...
                            <button type="button" wire:click="$set('showDeleteLevelButtons', false)" class="no-btn">
                                No
                            </button>
                        </div>
                    @endif
    ...
    @if($showMapModal)
        <div wire:click="$set('showMapModal', false)" class="map_overlay">
            <div wire:click.stop class="map_popup relative max-w-3xl w-full" x-data="mapComponent2()" x-init="initMap()">
