...
<div x-data="{
    ...
    }" x-init="update(); window.addEventListener('resize', update)" class="absolute h-screen">
    ...
    <!-- Overlay with blur -->
    <div x-cloak x-show="open" @click="open = false" x-transition:enter="transition-opacity ease-linear duration-300"
        x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
        x-transition:leave="transition-opacity ease-linear duration-300" x-transition:leave-start="opacity-100"
        x-transition:leave-end="opacity-0" class="fixed inset-0 overlay-blur z-998"></div>
